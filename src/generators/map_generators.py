...
def create_mapbox_html(geojson_data, title="Visor GeoJSON Profesional", folder_name="Proyecto", grouping_mode="layer", point_color="#ff0000", line_color="#0000ff"):
    ...
    function initializeMap() {{
      try {{
        ...
        function loadDataToMap() {{
          const layers = {{}};
          const layersWithTexts = new Set();
          currentGeoJSON.features.forEach(f => {{
              // Usar layer si existe, si no usar type, si no usar 'Defecto'
              const l = f.properties.layer || f.properties.type || 'Defecto';
              if (!layers[l]) layers[l] = [];
              layers[l].push(f);
              // Detectar si esta capa tiene textos
              if (f.properties && f.properties.type === 'text') {{
                  layersWithTexts.add(l);
              }}
          }});
          ...
          // Crear entrada para cada capa (geometría + textos por separado)
          // Usar colores iniciales pasados desde Python
          const initialLineColor = '{line_color}';
          const initialPointColor = '{point_color}';
          
          layersList.forEach(l => {{
              // Determinar si la capa tiene líneas o puntos para asignar el color apropiado
              const hasLines = layers[l].some(f => {{
                  const t = f.properties && f.properties.type;
                  return ['line', 'polyline', 'polygon', 'shape', 'track', 'LineString'].includes(t);
              }});
              const defaultColor = hasLines ? initialLineColor : initialPointColor;
              layerColors[l] = layerColors[l] || defaultColor;
              
              // Capa principal (puntos y líneas)
              ...
              // Capa de textos (si esta capa tiene textos)
              if (layersWithTexts.has(l)) {{
                  const textDiv = document.createElement('div');
                  textDiv.className = 'checkbox-group';
                  textDiv.style.marginLeft = '20px';
                  textDiv.style.fontSize = '11px';
                  textDiv.innerHTML = `<input type="checkbox" id="sh-${{l}}-texts" checked><label for="sh-${{l}}-texts">[TXT] ${{l}}</label>`;
                  control.appendChild(textDiv);
                  document.getElementById(`sh-${{l}}-texts`).onchange = loadLayers;
              }}
          }});
        ...
        function loadLayers() {{
          ...
          layersList.forEach(l => {{
              ...
              // Filtro para verificar layer o type (para compatibilidad con GPX/KML)
              const layerFilter = ['any', ['==', ['get','layer'], l], ['==', ['get','type'], l]];
              ...
              map.addLayer({{ 
                  ...
                  type: 'circle', 
                  source: 'src', 
                  filter: ['all', layerFilter, ['in', ['get','type'], ['literal', ['point','block']]]], 
                  paint: {{ 'circle-color': col, 'circle-radius': 5 }} 
              }});
              
              // Capa de líneas (incluye track de GPX)
              map.addLayer({{ 
                  id: `${{l}}-lns`, 
                  type: 'line', 
                  source: 'src', 
                  filter: ['all', layerFilter, ['in', ['get','type'], ['literal', ['line','polyline','polygon','shape','track','LineString']]]], 
                  paint: {{ 'line-color': col, 'line-width': 2 }} 
              }});
              
              // Capa de textos (solo si el checkbox específico está activado)
              const textCheckbox = document.getElementById(`sh-${{l}}-texts`);
              if (textCheckbox && textCheckbox.checked) {{
                  map.addLayer({{ 
                      id: `${{l}}-txt`, 
                      type: 'symbol', 
                      source: 'src', 
                      filter: ['all', layerFilter, ['==', ['get','type'], 'text']], 
                      layout: {{ 'text-field': ['get','text'], 'text-size': 12, 'text-anchor': 'top' }}, 
                      paint: {{ 'text-color': col, 'text-halo-color': '#fff', 'text-halo-width': 1 }} 
                  }});
              }}
          }});
        }}
